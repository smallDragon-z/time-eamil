...
// 默认成功响应
function commonRes<T>(res: Response, data: T, options?: ResOption) {
  ...
  if (resStatus === undefined) {
    // 根据状态设置状态码
    resStatus = type === Code[200] ? 200 : 404;
  }

  // 响应参数
  const sendRes: { code: number; data: unknown; message?: object | string } = {
...
commonRes.error = function (res: Response, data: unknown, message?: unknown, status?: number) {
  ...
  this(res, data, { type: 'error', message: message || CodeMessage['error'], status: status || 404 });
};
